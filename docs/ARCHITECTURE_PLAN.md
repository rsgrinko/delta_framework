...
### Этап 5 — выполнен частично 2026-08-14

Этап разделён: сначала безопасность и корректность (сделано), затем декомпозиция моделей (не сделано).

Сделано:

- **`Delta\Security\PasswordHasher` — переход на bcrypt с прозрачной миграцией.** Старые md5-хеши принимаются
  при входе и перехешируются в bcrypt при первом же успешном входе; сброс паролей не требуется. Поиск по хешу
  пароля заменён на выборку по логину плюс проверку — и в `securityAuthorize()`, и в `ApiController::getToken()`.
- Отпечаток сессии и токен автологина вынесены в отдельные методы, сверка идёт через `hash_equals()`.
  Токен автологина построен на `hash_hmac`, а не на конкатенации с md5.
- Коды верификации генерируются через `random_bytes()`: прежде они выводились из статической соли, которая
  утекла в историю git, то есть были предсказуемы.
- **Транзакция вокруг создания пользователя.** Вставка пользователя и назначение роли стали неделимой
  операцией; при ошибке — откат. Прежде падение на втором шаге оставляло пользователя без роли.
- **Исправлена инвалидация кэша в `User::update()`.** Ключ, который удалялся, не совпадал ни с одним реально
  используемым: данные кэшируются под двумя ключами (с флагом `onlySecureData` и без), удалялся третий.
  Сейчас безвредно, так как `USE_CACHE` выключен, но при включении отдавались бы устаревшие пароль, email и роли.
- Тесты: `PasswordHasherTest`, включая сценарий миграции со старой схемы. Всего в проекте 70 тестов.

**Побочный эффект развёртывания:** формат отпечатка сессии и токена автологина изменился, поэтому все активные
сессии и cookie «запомнить меня» становятся недействительными — пользователям потребуется войти заново один раз.

**Не сделано:**

- DI-контейнер и перевод сервисов на внедрение зависимостей.
- Декомпозиция `User` (1040 строк) на сущность, репозиторий и сервисы, и то же для `Dialog`, `Posts`, `File`,
  `Roles`. Это работа на несколько дней, и делать её без предварительного покрытия тестами текущего поведения
  авторизации и профиля — значит менять непокрытый код вслепую. Разумный порядок: сначала интеграционные тесты
  на эти сценарии, затем декомпозиция.
- Проверка успешного входа с реальным паролем на живом хосте: у исполнителя нет учётных данных. Логика
  миграции покрыта юнит-тестами, но **сквозную проверку входа должен выполнить владелец проекта.**

Проверка: линт 8.4, `phpunit` — 70 тестов зелёные, ответы всех маршрутов сайта и админки в норме,
отказ при неверном пароле отрабатывает корректно.

### Интеграционные тесты входа и профиля — 2026-08-14

Появилась инфраструктура тестов против настоящей базы — то, без чего декомпозицию моделей нельзя было
начинать: менять непокрытый код авторизации значило бы работать вслепую.

- `phpunit.xml` с двумя наборами: `unit` (без базы) и `integration`.
- `tests/bootstrap.php` берёт реквизиты из `config.local.php` и переопределяет **только** префикс таблиц
  на `test_`. Прав на создание отдельной базы у учётной записи нет (проверено `SHOW GRANTS`), поэтому
  изоляция достигается префиксом: тесты создают собственные таблицы в той же базе и удаляют их после прогона.
  Префикс определяется до загрузки классов ядра — имена таблиц вычисляются в константах классов при первой
  загрузке файла.
- `tests/Support/TestDatabase.php` — схема, очистка и помощники для вставки данных мимо моделей.
- `tests/Integration/AuthTest.php` (11 тестов): успешный вход, отказ по неверному паролю и неизвестному логину,
  **приём старых md5-хешей и их автоматическая миграция в bcrypt**, повторный вход после миграции,
  инвалидация сессии при смене пароля в обход неё, выход, обновление `last_active`.
- `tests/Integration/ProfileTest.php` (11 тестов): чтение профиля, сокрытие пароля и токена в безопасных
  данных, сброс кэша при записи, смена пароля и отказ по неверному текущему, **сохранение сессии после смены
  пароля** и две смены подряд, смена пароля сразу после миграции хеша, вход с новым паролем, роли и признак
  администратора.

Это закрывает пункт, который на Этапе 5 был помечен как «должен проверить владелец»: путь
«старый хеш → вход → перехеширование → повторный вход» теперь подтверждается автоматически.

Найдено по ходу: `User::logout()` невозможно вызвать из CLI — он дёргает `setcookie()` напрямую, а это
ошибка после любого вывода. Добавлена проверка `headers_sent()`: установка cookie после отправки вывода
всё равно ничего не делает. Сама необходимость такой заплатки — наглядная иллюстрация того, зачем модель
нужно избавлять от прямой работы с cookie.

Приведён в порядок `.gitlab-ci.yml`: убраны фиктивные задания (`sleep 60`, напечатанное «покрытие 90%»),
добавлены раздельные прогоны наборов и проверка синтаксиса. Без `config.local.php` интеграционный набор
корректно пропускается, а не роняет процесс.

Выполнена отложенная чистка базы (`db/cleanup/2026-08-14_stage0.sql`): удалены две пустые таблицы-остатки
и 25 сниппетов кода удалённой веб-консоли. Резервная копия сниппетов снята перед удалением.

Итого тестов в проекте: **92**.

### Этап 5 (часть 2): выделение AuthService — 2026-08-14

Первый шаг декомпозиции моделей, выполненный уже под прикрытием интеграционных тестов.

- `Core\Services\AuthService` забрал у модели всё, что не относится к данным: проверку пароля, наполнение
  сессии, выдачу и сверку cookie автологина, выход, отпечаток сессии, перехеширование при входе.
- Сервис **не статический** и принимает соединение, хешер, имя таблицы и соль через конструктор — то есть
  поведение авторизации теперь можно проверить с другим соединением или другим хешером.
- `User::securityAuthorize()`, `authorize()`, `isAuthorized()`, `getCurrentUserId()`, `logout()`,
  `sessionFingerprint()` остались как тонкие делегаты: на них опирается существующий код, и ломать его
  ради чистоты нет смысла. Новый код должен обращаться к сервису напрямую.
- Из модели удалены `setAuthCookie()`, `forgetAuthCookie()`, `authCookieToken()`, `rehashIfNeeded()` —
  они переехали в сервис. `User.php` сократился с 1172 до 1041 строки и больше не работает с cookie вообще.

Тесты не менялись ни на строку и все 92 остались зелёными — это и есть подтверждение, что поведение
сохранено. Ровно для этого они и писались.

### Этап 5 (часть 3): UserRepository и RegistrationService — 2026-08-14

- **`Core\Repositories\UserRepository`** — весь SQL по пользователям собран в одном месте. Модель больше не
  знает ни про имя таблицы, ни про формат условий выборки. Заодно в `paginate()` приведены к безопасному виду
  `LIMIT` и направление сортировки: они часть синтаксиса запроса и не могут идти плейсхолдерами.
- **`Core\Services\RegistrationService`** — создание пользователя: проверка уникальности логина и почты,
  транзакция вокруг вставки и назначения роли, рассылка **за пределами** транзакции. Прежде неудачная отправка
  письма происходила внутри общего потока и путала причину отказа с причиной незавершённой записи.
- Из модели убраны все прямые обращения к `DataBase` кроме фабрик сервисов и трёх мест, относящихся к другим
  сущностям (таблица ролей, фильтр выборки) — они переедут вместе со своими моделями.

`User.php`: 1041 → 995 строк. Суммарно за декомпозицию — с 1172 до 995, при этом 743 строки выделенного кода
теперь лежат в трёх классах с явными зависимостями и поддаются подмене в тестах.

Тесты снова не менялись: все 92 зелёные, сайт проверен.

**Следующие шаги** (отложены на следующую сессию): `ProfileService` (смена пароля и email), DI-контейнер,
чтобы сервисы перестали создаваться фабричными методами модели, затем декомпозиция `Dialog`, `Posts`, `File`,
`Roles` по той же схеме. После этого — Этап 6: физическое разделение `src/` и `app/`, консольные команды и
устранение race condition в очереди MQ.
