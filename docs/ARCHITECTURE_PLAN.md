...
Следующие шаги декомпозиции: `UserRepository` (SQL), `RegistrationService` (`User::create()` с рассылкой),
`ProfileService` (смена пароля и email), затем DI-контейнер, чтобы сервисы получали зависимости не через
